...
export default function UploadSection({ onUploaded }) {
  ...
  useEffect(() => {
    const fetchSamples = async () => {
      try {
        const { data } = await api.get("/api/v1/sample-datasets");
        setSampleDatasets(data);
      } catch (e) {
        console.error("Failed to load sample datasets", e);
      }
    };
    fetchSamples();
  }, []);
  ...
  const loadSample = async (datasetId, datasetName) => {
    setLoadingSample(datasetId);
    setError("");
    try {
      const { data } = await api.post(`/api/v1/sample-datasets/${datasetId}/load`);
  ...
  const domainColors = {
    hiring: "rgba(96,165,250,0.4)",
    healthcare: "rgba(52,211,153,0.4)",
    lending: "rgba(251,191,36,0.4)",
    insurance: "rgba(251,146,60,0.4)",
  };
      ...
      <div style={containerStyle}>
        ...
        {sampleDatasets.length > 0 && (
          <motion.div
            ...
            style={sampleSectionStyle}
          >
            <p style={sampleLabelStyle}>Or try a sample dataset</p>
            <div style={sampleGridStyle}>
              {sampleDatasets.map((ds) => (
                <motion.button
                  key={ds.id}
                  onClick={() => loadSample(ds.id, ds.name)}
                  disabled={loadingSample !== null}
                  whileHover={{ scale: 1.02, borderColor: "rgba(255,255,255,0.2)" }}
                  whileTap={{ scale: 0.98 }}
                  style={{
                    ...sampleCardStyle,
                    borderColor: domainColors[ds.domain] || "rgba(255,255,255,0.1)",
                    background: loadingSample === ds.id
                ...
                >
                  {loadingSample === ds.id ? (
                    ...
                    <>
                      <p style={sampleCardTitleStyle}>{ds.name}</p>
                      <p style={sampleCardDescStyle}>{ds.description}</p>
                      <p style={sampleCardMetaStyle}>
                        {ds.rows} rows · Outcome: {ds.outcome_column}
                      </p>
                    </>
                  )}
                </motion.button>
